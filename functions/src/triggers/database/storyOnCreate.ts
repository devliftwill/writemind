  ...
  .onCreate(
    async (
      ...
      try {
        ...
        const textStory = await createCompletion(
          `write a 4 pharagraph story about ${story?.text}`,
        ...
        const audioResp = await elevenlabsTextToSpeech(
          text as string,
          story?.voice_id || "21m00Tcm4TlvDq8ikWAM"
        );
